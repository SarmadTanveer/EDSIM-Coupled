...
def app():
    ...
    with st.form("CreateScenario"): 
        ...
        st.header("Patient Inter-Arrival Time In Mins")
        col01, col02 = st.columns(2)
        with col01:
            walkInInter = st.number_input('Walk-In Patients', 1.0, 120.0, 25.0, step= 0.1,help="Min=1, Max=120")
        
        with col02:
            ambulanceInter = st.number_input('Ambulance Patients',1.0,120.0,25.0,step=0.1,help="Min=1, Max=120")
        ...
        with col13:
            CTASass_std_dev = st.number_input('CTAS Assessment (STD)', 1.0, 60.0, 10.0,step=0.1)
            Priorityass_std_dev = st.number_input('Priority Assessment (STD)', 1.0, 60.0, 5.0,step=0.1)
            Initialass_std_dev = st.number_input('Initial Assessment (STD)', 1.0, 60.0, 25.0,step=0.1)
            Dischargeass_std_dev = st.number_input('Discharge (STD)', 1.0, 60.0, 20.0,step=0.1)
        with col14:
            CTASass = st.number_input('CTAS Assessment (Mean)', 1.0, 120.0, 15.0,step=0.1,)
            Priorityass = st.number_input('Priority Assessment (Mean)', 1.0, 120.0, 10.0,step=0.1)
            Initialass = st.number_input('Initial Assessment (Mean)', 1.0, 120.0, 15.0,step=0.1)
            Dischargeass = st.number_input('Discharge (Mean)', 1.0, 50.0, 23.0,step=0.1)
        with col15:
            Treatment_std_dev = st.number_input('Treatments (STD)', 1.0, 120.0, 60.0,step=0.1)
            Bedass_std_dev = st.number_input('Bed Assignment (STD)', 1.0, 60.0, 10.0,step=0.1)
            Resus_std_dev = st.number_input('Resuscitations (STD)', 1.0, 60.0, 20.0,step=0.1)
            Registration_std_dev = st.number_input('Registrations (STD)', 1.0, 60.0, 10.0,step=0.1)
        with col16:
            Treatment = st.number_input('Treatments (Mean)', 1.0, 180.0, 75.0,step=0.1)
            Bedass = st.number_input('Bed Assignment (Mean)', 1.0, 60.0, 10.0,step=0.1)
            Resus = st.number_input('Resuscitations (Mean)', 1.0, 120.0, 60.0,step=0.1)
            Registration = st.number_input('Registrations (Mean)', 1.0, 60.0, 10.0,step=0.1)
...
def createScenario(scenario): 
    if(formValidation(scenario['name'],scenario['ctasDist'])):
        ...
        del scenarioList
        st.success('Scenario created and added to scenario list')
...
def formValidation(name,ctasDist):
    ...
    for scenario in scenarioList:
        if scenario['name'] == name:
            st.error('Duplicate scenario names are not allowed')
            return False
    del scenarioList
    ...
    if(not name):
        ...
        st.erroe('Walkin CTAS distribution does not add upto 100')
        return False
    elif(ambulanceSum != 1):
        st.error('Ambulance CTAS distribution doesnot add upto 100')
        return False
    else: 
        return True
